---
title: "IE48B Homework 2 Rmd- SÜLEYMAN TURANCAN BURUCU"
...
>In brief, this homework explains how we can construct some penalized regression approaches for time series. First of all, we have constrcuted 1D Fused Lasso representation with k = 10 and found the best lambda values for each time series. Then, we have constructed regression tree with different maxdepth values. Then, we have calculated the MSE values between for each time series on the dataset for 1D Fused Lasso model with best lambda value and the regression tree model with best maxdepth value. Moreover, we have created 1-nearest-neighborhood (1-NN) model. Also, we have checked the Euclidian distances between the excluded points for each model; 1D Fused Lasso, regression tree, and 1-NN. Finally, we have predicted the excluded points' classes and checked whether we made predictions correct or not, and calculated accuracy values fore ach class

### **Question - 1**

<p>Firstly, we are going to exclude each 1 observation from 10 observations, i.e. k = 10, for each time series to test 1D Fused Lasso model on the excluded data. Yet, exclusion didn't mean that we have excluded the point permanently, we have just used these excluded points on our representations to check our representations and their results. Then, we needed to find the optimal lambda values for each time series to complete 1D Fused Lasso model. Yet, as you can see from the plots, the minimum lambda values didn't give the best result since it causes overly fitted values for the predicted values; therefore, we had to use 1-standard-error rule and use the lambda values giving 1-standard-error values. Let's check:
</p>
...
par(mfrow = c(1,2))
plot(out1, lambda=cv1$lambda.min, main="Minimal CV error for the 1st Time Series") 
plot(out1, lambda=cv1$lambda.1se, main="One standard error rule for the 1st Time Series")
#cv1$lambda.min#overfit olduğu için bu iyi durmuyor, zaten değer de 0 a yakın baya
#cv1$lambda.1se#overfit olmadığı için bu iyi duruyor
...
colnames(t) = c("1D Fused Lasso Model for Time Series", "Best Lambda Values")
t
...
<p>After creating 1D Fused Lasso model, we have tried our best to represent each time series with regression trees. These regression tree models are chosen by checking the models with the least standard error (xerror) among the models that have maxdepth=1,2,3,4,5,6,7,8 values and cp=0. Let's look at the models:
</p>
```{r, eval = TRUE, error=FALSE, warning=FALSE, message = FALSE, fig.width = 12, include = TRUE, echo = FALSE}
knitr::opts_chunk$set(echo = TRUE)
#QUESTION - 2
...
<p>We have found the best lambda values for 1D Fused Lasso model and found the best maxdepth values for regression tree model. Then, we are going to calculate the MSE values between the predicted values of each time series both found by 1D Fused Lasso Model and regression tree model and the exact value of each time series. Then, we are going to draw the boxplot of the MSE values of 1D Fused Lasso model and the boxplot of the regression tree model which will be calculated by the MSE function giving the mean squared error between the exact values and the predicted values for each representation for each time series. Let's see:
</p>
...
<p>We have calculated best lambda values for 1D Fused Lasso model, best maxdepth values for regression tree model, MSE values of 1D Fused Lasso model, and regression tree model for each time series. Then, we were requested to calculate and predict the classes of the excluded values in accordance with what we have learnt through different representations. Let's check the accuracy of the representations (codes are included in as (#...) form in the code, shown below are just the results): 
</p>
